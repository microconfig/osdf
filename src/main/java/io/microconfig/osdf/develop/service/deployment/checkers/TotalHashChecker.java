...
import io.microconfig.osdf.develop.cluster.resource.totalhash.TotalHashesStorage;
...
import static io.microconfig.osdf.develop.cluster.resource.totalhash.TotalHashComputer.totalHashComputer;
import static io.microconfig.osdf.develop.cluster.resource.totalhash.TotalHashesStorage.totalHashesStorage;
...
public class TotalHashChecker {
    ...
    public boolean check(ServiceDeployment deployment, ServiceFiles files) {
        ...
        if (NewImageVersionChecker.imageVersionChecker(deployment, files, paths).isLatest()) {
            info("Up-to-date");
        } else {
